...
function generate_csp(html) {
  ...
  const csp_hashes = $('script[type="module"]').map((i, el) => hash_script($(el).get()[0].children[0].data)).toArray().map((h) => `'sha256-${h}'`).join(" ");
...
export default function({ pages = "build", assets = pages, fallback = void 0, manifestVersion = 3 } = {}) {
  return {
